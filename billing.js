(() => {
  ...
  const nextNumber = (type) =>
    `${type === "Devis" ? "DEV" : "FAC"}-${new Date().getFullYear()}-${String(docs.filter((doc) => doc.type === type).length + 1).padStart(3, "0")}`;
  ...
  const rows = (list) =>
    list
      .map(
        (document) =>
          `<div class="billing-row"><div><strong>${document.number} · ${document.client}</strong><small>${document.title}</small></div><div><strong>${euro(document.amount)}</strong><small class="invoice-status">${document.status}</small></div>${actions(document)}</div>`,
      )
      .join("") || '<div class="empty">Aucun document.</div>';
  function render() {
    const quotes = docs.filter((doc) => doc.type === "Devis"),
      invoices = docs.filter((doc) => doc.type === "Facture"),
      due = invoices
  ...
  function form(document = null, type = "Devis") {
    ...
    const statusOptions =
      documentType === "Devis"
        ? ["À envoyer", "Accepté", "Refusé", "Facturé"]
        : ["En attente", "Réglée", "En retard"];
    ...
    dialog.querySelector("form").onsubmit = (event) => {
      ...
      if (document)
        Object.assign(document, values, { amount: Number(values.amount) });
      else
        docs.push({
          ...values,
          id: Date.now(),
          type: documentType,
          number: nextNumber(documentType),
  ...
  window.document.addEventListener("click", (event) => {
    ...
    if (create) form(null, create.dataset.newDocument);
    const edit = event.target.closest("[data-edit-document]");
    if (edit)
      form(docs.find((document) => document.id == edit.dataset.editDocument));
    ...
    if (remove) {
      ...
      if (
        ...
        docs = docs.filter((item) => item.id != document.id);
        save();
        render();
      }
    }
    ...
    if (paid) {
      const invoice = docs.find((document) => document.id == paid.dataset.paid);
      if (invoice) {
